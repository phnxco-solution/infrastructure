 ...
 *
 * TLS terminates at Traefik and reaches php-fpm as plain http. Laravel trusts no
 * proxies unless bootstrap/app.php says so, and the resulting breakage is invisible
 * locally and in CI — it only appears once deployed.
 *
 ...
 *     -v "$PWD/probe-proxy.php:/tmp/probe.php:ro" \
 *     <image> php /tmp/probe.php
 *
 *   # optional: name a signed route to test (default: guesses from the route list)
 *   ... php /tmp/probe.php <signed.route.name> <param>=<value>
 *
 * PASS means: forwarded IP surfaces, isSecure() is true, and a signed URL generated
 * from APP_URL still validates on arrival. Any FAIL => add to bootstrap/app.php:
 *
...
function forwarded(string $url): Request
{
    ...
    (new TrustProxies)->handle($request, fn ($r) => null);
...
// Signed URLs: the failure that kills emailed links. Generated from APP_URL (https,
// in a queued Mailable), verified against the inbound request's rebuilt URL.
...
if ($routeName !== null) {
    try {
        ...
        $inbound = forwarded(str_replace('https://', 'http://', $signed));
